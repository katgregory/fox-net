...
for component in ${M64P_COMPONENTS}; do
	...
	#"$MAKE" -C source/mupen64plus-${component}/projects/unix all -j6 $@
	#"$MAKE" -C source/mupen64plus-${component}/projects/unix -j6 install $@ ${MAKE_INSTALL} DESTDIR="$(pwd)/build/"

	#mkdir -p ./build/doc
	#for doc in LICENSES README RELEASE; do
#		if [ -e "source/mupen64plus-${component}/${doc}" ]; then
#			cp "source/mupen64plus-${component}/${doc}" "./build/doc/${doc}-mupen64plus-${component}"
#		fi
#	done
...
#			cp "source/mupen64plus-${component}/doc/${subdoc}" ./build/doc/
#		fi
#	done
done
