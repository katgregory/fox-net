...
void frender()
{
  // outputVideo.open("dump.mp4", CV_FOURCC('M','P','4','2'), 60, cv::Size(640,480), true);
...
//  qqq=0;
  ...
  //cv::namedWindow( "dump", CV_WINDOW_AUTOSIZE );
}
// ref rate
...
//INSERT 231 HERE
  frender();
  printf("--------------File Renderer Initialized\n");
...
void newSwapBuffers()
{
  ...
  // //cv::Mat mf2(480, 640, CV_8UC3, cv::Scalar(0));
  // //std::cout<<"X"<<settings.res_x<<"Y"<<settings.res_y<<std::endl;
  //std::cout.flush();
...
//  qqq++;
  ...
  //       {
  //         col = src[x];
  //         // r = (wxUint8)((col >> 19) & 0x1F)<<3;
  //         // g = (wxUint8)((col >> 10) & 0x3F)<<2;
  //         // b = (wxUint8)((col >> 3)  & 0x1F)<<3;
  //         r = (wxUint8)((col >> 16) & 0xFF);
  //         g = (wxUint8)((col >> 8) & 0xFF);
  //         b = (wxUint8)((col >> 0)  & 0xFF);
  //         //col=0;
  //         cv::Point3_<char>* p = matframe.ptr<cv::Point3_<char> >(y,x);
  //         p->x=b; //B
  //         p->y=g; //G
  //         p->z=r; //R
  //         if(qqq==1000)
  //         {
  //          // myfile << ((int)(col>>24))<<","<<((int)r)<<","<<((int)g)<<","<<((int)b)<<"\n";
  //         }
  //         //dst[x] = (r<<11)|(g<<5)|b;
  //       }
  //     offset_dst += settings.res_x << 1;
  ...
  //   // Unlock the backbuffer
  //   grLfbUnlock (GR_LFB_READ_ONLY, GR_BUFFER_BACKBUFFER);
  //   // cvtColor(matframe,mf2,CV_BGR2YCbCr);
    //mshow("dump",matframe);
    //cv::waitKey(1);
    //std::cout<<".";
    //std::cout.flush();
