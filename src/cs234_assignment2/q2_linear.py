...
class Linear(DQN):
    ...
    def get_q_values_op(self, state, scope, reuse=False):
        ...
        out = tf.contrib.layers.linear(inputs=out, num_outputs=num_actions, scope=scope, reuse=reuse)
        ##############################################################
        ######################## END YOUR CODE #######################
    ...
    def add_update_target_op(self, q_scope, target_q_scope):
        ...
        col_q = tf.get_collection(key=tf.GraphKeys.TRAINABLE_VARIABLES, scope=q_scope)
        col_tarq = tf.get_collection(key=tf.GraphKeys.TRAINABLE_VARIABLES, scope=target_q_scope)
        assigns = []
        for i in range(len(col_q)):
                assigns.append(tf.assign(col_tarq[i], col_q[i])) 
    ...
    def add_loss_op(self, q, target_q):
        ...
        self.loss = 0
        Q_samp = self.r + (1-tf.cast(self.done_mask, dtype=tf.float32))*self.config.gamma*tf.reduce_max(target_q,axis=1)

        action = tf.one_hot(indices=self.a, depth=num_actions)


        self.loss = tf.reduce_sum((Q_samp-tf.reduce_sum(q*action, axis=1))**2)
#        self.loss = tf.reduce_sum(tf.square(Q_samp - tf.reduce_sum(q*tf.one_hot(self.a, num_actions), axis=1)))
    ...
    def add_optimizer_op(self, scope):
        ...
        # Clip the gradients by norm with self.config.clip_val if self.conf.grad is true
        if self.config.grad_clip:
            for i in range(len(grads_and_vars)):
                    grad_clip = tf.clip_by_norm(grads_and_vars[i][0], clip_norm = self.config.clip_val)
                    grads_and_vars[i] = (grad_clip, grads_and_vars[i][1])
        # Apply the gradients and store the train op in self.train_op
        self.train_op = optimizer.apply_gradients(grads_and_vars = grads_and_vars)
        
        # Compute the global norm of the gradients and store this scalar in self.grad_norm
        self.grad_norm = tf.global_norm([gradient for gradient, _ in grads_and_vars])
